...
class RootSTCFMeasurementReader_BKG : public IReader {
 public:
  ...
  struct Config {
    ...
    std::vector<std::string> v_treeName = {
        "events", "info_withBKG"};  ///< name of the input tree
  ...
  RootSTCFMeasurementReader_BKG(const Config& config,
  ...
  std::string name() const final override {
    return "RootSTCFMeasurementReader_BKG";
  }
